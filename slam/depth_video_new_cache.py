...
def get_video_dataset(opt):
    ...
    elif opt.dataset_name == 'mono':
        return MonoVideoDataset(opt)
    else:
    ...
    def read_images(self):
        ...
        for i in range(1, self.number_of_frames+1):
            if self.opt.not_load_mask:
                seg = np.ones([H_opt, W_opt], dtype=np.float32)
            else:
...
class DavisLocalVideoDataset(DepthVideoDataset):
    ...
    def get_paths(self, opt):
        track_name = opt.track_name
        data_list_root = pathlib.Path(davis_path) / "JPEGImages/480p"
        mask_root = pathlib.Path(davis_path) / "Annotations/480p"
    ...
    def read_images(self):
        ...
        for img_path in tqdm(image_paths):
            img_raw = np.asarray(Image.open(
                img_path).convert('RGB')).astype(np.float32)/255
            if not hasattr(self, 'original_shape'):
    ...
    def read_gt_info(self):
        if self.opt.not_load_mask:
            ...
            for mask_path in mask_paths:
                mask = np.asarray(Image.open(
                    mask_path).convert('L')).astype(np.float32)/255
        ...
        #     extrinsics = np.linalg.inv(np_file['w2c']).astype(np.float32)
...
class TUMVideoDataset(DepthVideoDataset):
    ...
    def read_images(self):
        print('reading images...')
        img_path = self.paths['image_path']
        self.images_raw = np.load(img_path).astype(np.float32)/255
        if self.opt.frame_cap is not None:
...
class MonoVideoDataset(DepthVideoDataset):
    def __init__(self, opt):
        super().__init__()
        self.get_paths(opt)
        self.initialize(opt)
    ...
    def get_paths(self, opt):
        track_name = opt.track_name
        image_path = pathlib.Path(opt.mono_data_root)
        self.paths = {'image_path': image_path,
                      'mask_path': None}
